...
public class FaceNetModel {
    ...
    private static final String MODEL_NAME = "facenet_quant.tflite";

    private static final int BATCH_SIZE = 1;
    public static final int IMG_HEIGHT = 160;
    ...
    public FaceNetModel(Activity activity) throws IOException {
    ...
    private void convertBitmapToByteBuffer(Bitmap image) {
        Bitmap bitmap = Bitmap.createScaledBitmap(image, 160, 160, false);
        ...
        for (int i = 0; i < IMG_WIDTH; ++i) {
            for (int j = 0; j < IMG_HEIGHT; ++j) {
                ...
                imageData.putFloat((((val >> 16) & 0xFF)-128f)/128f);
                imageData.putFloat((((val >> 8) & 0xFF)-128f)/128f);
                imageData.putFloat((((val) & 0xFF)-128f)/128f);
            }
